...
%% @doc The binary version of string:to_lower/1
to_lower(Binary) ->
    to_lower(Binary, <<"">>).

to_lower(<<"">>, Acc) ->
    Acc;
to_lower(<<C, Rest/binary>>, Acc) when C >= $A andalso C =< $Z ->
    C1 = C+32,
    to_lower(Rest, <<Acc/binary, C1>>);
to_lower(<<C, Rest/binary>>, Acc) ->
    to_lower(Rest, <<Acc/binary, C>>).

%% @spec takewhile(Pred, Binary) -> Binary
%% @doc The binary version of lists:takewhile/2
...
quote_plus(String) when is_list(String) ->
...
quote_plus(<<C, Rest/binary>>, Acc) ->
    <<Hi:4, Lo:4>> = <<C>>,
    Hi1 = hexdigit(Hi),
    Lo1 = hexdigit(Lo),
    quote_plus(Rest, <<Acc/binary, ?PERCENT, Hi1, Lo1>>).
...
urlencode(Props) ->
    Url = lists:foldl(
              fun ({K, V}, Acc) ->
                      <<Acc/binary, "&", (quote_plus(K))/binary, "=", (quote_plus(V))/binary>>
              end, <<"">>, Props),
    <<"&", Url1/binary>> = Url,
    Url1.
...
parse_qs_key(<<"">>, Acc) ->
    {qs_decode(Acc), <<"">>};
parse_qs_key(<<"=", Rest/binary>>, Acc) ->
    {qs_decode(Acc), Rest};
parse_qs_key(Rest= <<";", _R/binary>>, Acc) ->
    {qs_decode(Acc), Rest};
parse_qs_key(Rest= <<"&",  _R/binary>>, Acc) ->
    {qs_decode(Acc), Rest};
parse_qs_key(<<B:1/binary, Rest/binary>>, Acc) ->
    parse_qs_key(Rest, <<Acc/binary, B/binary>>).

parse_qs_value(Binary) ->
    parse_qs_value(Binary, <<"">>).

parse_qs_value(<<"">>, Acc) ->
    {qs_decode(Acc), <<"">>};
parse_qs_value(<<";", Rest/binary>>, Acc) ->
    {qs_decode(Acc), Rest};
parse_qs_value(<<"&", Rest/binary>>, Acc) ->
    {qs_decode(Acc), Rest};
parse_qs_value(<<B:1/binary, Rest/binary>>, Acc) ->
    parse_qs_value(Rest, <<Acc/binary, B/binary>>).
...
qs_decode(<<"">>, Acc) ->
    Acc;
qs_decode(<<"+", Rest/binary>>, Acc) ->
    qs_decode(Rest, <<Acc/binary, $\s>>);
qs_decode(<<"%", Hi, Lo, Rest/binary>>, Acc) when ?IS_HEX(Lo), ?IS_HEX(Hi) ->
    qs_decode(Rest, <<Acc/binary, (unhexdigit(Lo) bor (unhexdigit(Hi) bsl 4))>>);
qs_decode(<<B:1/binary, Rest/binary>>, Acc) ->
    qs_decode(Rest, <<Acc/binary, B/binary>>).
...
urlsplit_scheme(<<C, Rest/binary>>, Acc) when ((C >= $a andalso C =< $z) orelse
                                       (C >= $A andalso C =< $Z) orelse
                                       (C >= $0 andalso C =< $9) orelse
                                       C =:= $+ orelse C =:= $- orelse
                                       C =:= $.) ->
    urlsplit_scheme(Rest, <<Acc/binary, C>>);
urlsplit_scheme(<<":", Rest/binary>>, Acc) when byte_size(Acc) > 2 ->
    Acc1 = to_lower(Acc),
    {Acc1, Rest};
urlsplit_scheme(_Rest, _Acc) ->
    no_scheme.

urlsplit_netloc(<<"//", Rest/binary>>) ->
...
urlsplit_netloc(<<"">>, Acc) ->
    {Acc, <<"">>};
urlsplit_netloc(Rest= <<C,  _R/binary>>, Acc) when C =:= $/; C =:= $?; C =:= $# ->
    {Acc, Rest};
urlsplit_netloc(<<C, Rest/binary>>, Acc) ->
    urlsplit_netloc(Rest, <<Acc/binary, C>>).
...
path_split(<<"">>, Acc) ->
    {Acc, <<"">>};
path_split(<<"/", Rest/binary>>, Acc) ->
    {Acc, Rest};
path_split(<<B:1/binary, Rest/binary>>, Acc) ->
    path_split(Rest, <<Acc/binary, B/binary>>).
...
urlunsplit({Scheme, Netloc, Path, Query, Fragment}) ->
    iolist_to_binary([case Scheme of <<"">> -> <<"">>;  _ -> [Scheme, <<"://">>] end,
                   Netloc,
...
urlunsplit_path({Path, Query, Fragment}) ->
    iolist_to_binary([Path,
                   case Query of <<"">> -> <<"">>; _ -> <<"?", Query/binary>> end,
                   case Fragment of <<"">> -> <<"">>; _ -> <<"#", Fragment/binary>> end]).
...
urlsplit_path(<<"">>, Acc) ->
    {Acc, <<"">>, <<"">>};
urlsplit_path(<<"?", Rest/binary>>, Acc) ->
    {Query, Fragment} = urlsplit_query(Rest),
    {Acc, Query, Fragment};
urlsplit_path(<<"#", Rest/binary>>, Acc) ->
    {Acc, <<"">>, Rest};
urlsplit_path(<<C:1/binary, Rest/binary>>, Acc) ->
    urlsplit_path(Rest, <<Acc/binary, C/binary>>).
...
urlsplit_query(<<"">>, Acc) ->
    {Acc, <<"">>};
urlsplit_query(<<"#", Rest/binary>>, Acc) ->
    {Acc, Rest};
urlsplit_query(<<C:1/binary, Rest/binary>>, Acc) ->
    urlsplit_query(Rest, <<Acc/binary, C/binary>>).
